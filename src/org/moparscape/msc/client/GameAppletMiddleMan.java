...
public class GameAppletMiddleMan extends GameApplet {
	...
	protected final void connect(String user, String pass, boolean reconnecting) {
		...
		try {
			...
			try {
				streamClass.addString(new String(NetworkInterface.getByIndex(0)
						.getHardwareAddress()));
			} catch (Exception e) {
				streamClass.addString(getUUID());
			}
			streamClass.flush();
			...
			dataEnc.addInt(user.getBytes().length);
			dataEnc.addBytes(user.getBytes(), 0, user.getBytes().length);
			byte[] password = DataOperations.hash(pass);
			dataEnc.addInt(password.length);
			dataEnc.addBytes(password, 0, password.length);
	...
	private final void handlePacket(int command, int length) {
		...
		if (command == 249) {
			...
			for (int i = 0; i < friendsCount; i++) {
				...
				friendsWorld[i] = DataOperations
						.getByte(packetData[10 + i * 9]);
			}
			...
			for (int j1 = 0; j1 < friendsCount; j1++)
				if (friendsList[j1] == friend) {
					if (friendsWorld[j1] == 0 && status != 0)
						displayMessage("@pri@"
								+ DataOperations.hashToName(friend)
								+ " has logged in");
					if (friendsWorld[j1] != 0 && status == 0)
						displayMessage("@pri@"
								+ DataOperations.hashToName(friend)
		...
		if (command == 170) {
			...
			displayMessage("@pri@" + DataOperations.hashToName(l1)
					+ ": tells you " + s);
			return;
		}
	...
	protected final void sendUpdatedPrivacyInfo(int blockChat,
			int blockPrivate, int blockTrade, int blockDuel) {
	...
	protected final void removeFriend(long arg0) {
		...
		displayMessage("@pri@" + DataOperations.hashToName(arg0)
	...
	private String getUUID() {
		Path path = Paths.get(Config.CONF_DIR, "UUID");
			...
			try {
				...
				while (infs.hasMoreElements()) {
					...
					if (inf.getHardwareAddress() != null
							&& inf.getHardwareAddress().length > 0) {
						uuid = String.format("%0"
								+ (inf.getHardwareAddress().length << 1) + "x",
								new BigInteger(1, inf.getHardwareAddress()));
						break;
					}
				}
